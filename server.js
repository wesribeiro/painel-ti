...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        if (user.password === null) {
            return res.status(403).json({ message: 'Primeiro acesso. Defina uma senha.', firstLogin: true });
        }
    ...
    } catch (e) { res.status(500).json({ message: 'Erro no servidor durante o login.', error: e.message }); }
});
...
app.post('/api/auth/change-password', async (req, res) => {
    ...
    try {
        const db = await dbPromise;
        const user = await db.get('SELECT * FROM users WHERE id = (SELECT id FROM users WHERE username = ?)', [username]);
        ...
        if (user.password !== null) {
            ...
            if (!isPasswordCorrect) return res.status(403).json({ message: 'Senha atual incorreta.' });
        }

        // --- MUDANÇA: Gerar o hash da nova senha ---
        const newPasswordHash = await bcrypt.hash(newPassword, SALT_ROUNDS);
        
        await db.run('UPDATE users SET password = ? WHERE id = ?', [newPasswordHash, user.id]);
        res.status(200).json({ message: 'Senha alterada com sucesso.' });
    } catch (e) { res.status(500).json({ message: 'Erro ao alterar a senha.', error: e.message }); }
});

app.post('/api/auth/logout', (req, res) => res.status(200).json({ message: "Logout realizado." }));


// --- ROTAS GENÉRICAS DE LEITURA (GET) ---
const createGetRoute = (tableName, jsonFields = []) => async (req, res) => {
    try {
        const db = await dbPromise;
        ...
        res.json(data);
    } catch (err) { res.status(500).json({ message: `Erro ao buscar ${tableName}`, error: err.message }); }
};
...
app.get('/api/users', async (req, res) => {
     try {
        const db = await dbPromise;
        const users = await db.all('SELECT id, name, username, lastLogin, roleId, storeId FROM users');
        res.json(users);
    } catch (error) {
...
app.post('/api/users', async (req, res) => {
    ...
    try {
        const db = await dbPromise;
        const result = await db.run('INSERT INTO users (name, username, roleId, storeId) VALUES (?, ?, ?, ?)', [name, username, roleId, storeId || null]);
        res.status(201).json({ id: result.lastID, name, username, roleId, storeId });
    } catch (e) { res.status(500).json({ message: 'Erro ao criar usuário', error: e.message }); }
});
...
app.put('/api/users/:id', async (req, res) => {
    ...
    try {
        const db = await dbPromise;
        await db.run('UPDATE users SET name=?, username=?, roleId=?, storeId=? WHERE id=?', [name, username, roleId, storeId || null, req.params.id]);
        res.status(200).json({ message: 'Usuário atualizado' });
    } catch (e) { res.status(500).json({ message: 'Erro ao atualizar usuário', error: e.message }); }
});
app.delete('/api/users/:id', async (req, res) => {
    try {
        ...
        res.status(204).send();
    } catch (e) { res.status(500).json({ message: 'Erro ao deletar usuário', error: e.message }); }
});
...
app.post('/api/stores', async (req, res) => {
    ...
    try {
        const db = await dbPromise;
        const defaultConfig = JSON.stringify({ items: [], noChecklistDaysLimit: 5 });
        const result = await db.run('INSERT INTO stores (name, pdvNamingStart, checklistConfig) VALUES (?, ?, ?)', [name, pdvNamingStart, defaultConfig]);
        const newStore = await db.get('SELECT * FROM stores WHERE id = ?', [result.lastID]);
        res.status(201).json(newStore);
    } catch (e) { res.status(500).json({ message: 'Erro ao criar loja', error: e.message }); }
});
app.put('/api/stores/:id', async (req, res) => {
    const { name, pdvNamingStart } = req.body;
    try {
        const db = await dbPromise;
        await db.run('UPDATE stores SET name=?, pdvNamingStart=? WHERE id=?', [name, pdvNamingStart, req.params.id]);
        res.status(200).json({ message: 'Loja atualizada' });
    } catch (e) { res.status(500).json({ message: 'Erro ao atualizar loja', error: e.message }); }
});
app.delete('/api/stores/:id', async (req, res) => {
    try {
        ...
        res.status(204).send();
    } catch (e) { res.status(500).json({ message: 'Erro ao remover loja', error: e.message }); }
});
app.get('/api/stores/:id/pdvs-with-status', async (req, res) => {
    try {
        const db = await dbPromise;
        const pdvs = await db.all('SELECT * FROM pdvs WHERE storeId = ? ORDER BY CAST(number AS INTEGER)', [req.params.id]);
        for (const pdv of pdvs) {
            const lastStatus = await db.get(`SELECT sh.*, u.name as techName FROM statusHistory sh LEFT JOIN users u ON u.id = sh.techId WHERE sh.pdvId = ? ORDER BY sh.timestamp DESC LIMIT 1`, [pdv.id]);
            pdv.lastStatus = lastStatus || null;
        }
        res.json(pdvs);
    } catch (err) { res.status(500).json({ message: 'Erro ao buscar PDVs', error: err.message }); }
});
app.post('/api/stores/:id/pdvs', async (req, res) => {
    const { number } = req.body;
    try {
        const db = await dbPromise;
        const result = await db.run('INSERT INTO pdvs (number, storeId) VALUES (?, ?)', [number, req.params.id]);
        res.status(201).json({ id: result.lastID, number, storeId: req.params.id });
    } catch (e) { res.status(500).json({ message: 'Erro ao criar PDV', error: e.message }); }
});
app.delete('/api/pdvs/:id', async (req, res) => {
    try {
        ...
        res.status(204).send();
    } catch (e) { res.status(500).json({ message: 'Erro ao remover PDV', error: e.message }); }
});
app.post('/api/pdvs/:id/status-history', async (req, res) => {
    ...
    if (!statusId || !description || !techId) {
        return res.status(400).json({ message: 'Status, descrição e ID do técnico são obrigatórios.' });
    }
    ...
    try {
        ...
        const result = await db.run(
            ...
            [pdvId, statusId, description, techId, itemId || null, new Date().toISOString()]
        );
        
        const newEntry = await db.get('SELECT * FROM statusHistory WHERE id = ?', [result.lastID]);
        res.status(201).json(newEntry);

    } catch (error) {
        res.status(500).json({ message: 'Erro ao salvar novo status no histórico.', error: error.message });
    }
});
app.post('/api/pdv-items', async (req, res) => {
    ...
    try {
        const db = await dbPromise;
        const result = await db.run('INSERT INTO pdvItems (name) VALUES (?)', [name]);
        const newItem = await db.get('SELECT * FROM pdvItems WHERE id = ?', [result.lastID]);
        res.status(201).json(newItem);
    } catch (e) {
        if (e.code === 'SQLITE_CONSTRAINT') {
            return res.status(409).json({ message: 'Este item de PDV já existe.' });
        }
        res.status(500).json({ message: 'Erro ao criar item de PDV.', error: e.message });
    }
});
app.delete('/api/pdv-items/:id', async (req, res) => {
    try {
        const db = await dbPromise;
        const result = await db.run('DELETE FROM pdvItems WHERE id = ?', [req.params.id]);
        ...
        res.status(500).json({ message: 'Erro ao remover o item de PDV.', error: e.message });
    }
});
...
app.get('/api/pdvs/:id', async (req, res) => {
    try {
        const db = await dbPromise;
        const pdv = await db.get('SELECT * FROM pdvs WHERE id = ?', [req.params.id]);
        if (pdv) {
            res.json(pdv);
        } else {
        ...
        res.status(500).json({ message: 'Erro ao buscar dados do PDV', error: err.message });
    }
});
...
app.get('/api/pdvs/:id/history', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Erro ao buscar histórico do PDV', error: err.message });
    }
});
...
app.get('/api/pdvs/:id/recurring-problems', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Erro ao buscar problemas recorrentes', error: err.message });
    }
});
...
app.get('/api/checklists/today', async (req, res) => {
    try {
        ...
        if (!storeId) { return res.status(400).json({ message: "O ID da loja é obrigatório." }); }
        const db = await dbPromise;
        const today = new Date().toISOString().split('T')[0];
        const checklist = await db.get('SELECT * FROM checklists WHERE storeId = ? AND date = ?', [storeId, today]);
        if (checklist) {
    ...
    } catch (e) { res.status(500).json({ message: 'Erro ao buscar checklist do dia', error: e.message }); }
});
...
app.post('/api/checklists', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        let checklistId = id;
        if (checklistId) {
            await db.run('UPDATE checklists SET status=?, pdvChecks=?, finalizedByUserId=? WHERE id=?', [status, pdvChecksJson, finalizedByUserId, checklistId]);
        } else {
            const result = await db.run('INSERT INTO checklists (storeId, date, status, pdvChecks, finalizedByUserId) VALUES (?, ?, ?, ?, ?)', [storeId, date, status, pdvChecksJson, finalizedByUserId]);
        ...
        if (status === 'completed' && pdvChecks) {
            ...
            for (const check of pdvChecks) {
                if (check.newStatusId) {
                    const description = `[CHECKLIST] ${check.observation || (check.result === 'ok' ? 'Tudo OK.' : 'Problema reportado.')}`;
                    console.log(`Inserindo log para PDV ${check.pdvId}: Status ${check.newStatusId}`);
                    await stmt.run(check.pdvId, check.newStatusId, description, userFromToken.id, new Date().toISOString());
                }
            }
        ...
        res.status(id ? 200 : 201).json({ id: checklistId, message: 'Checklist salvo com sucesso.' });

    } catch (e) {
        const db = await dbPromise;
        await db.run('ROLLBACK').catch(() => {});
        console.error("ERRO AO SALVAR CHECKLIST:", e);
        res.status(500).json({ message: 'Erro ao salvar checklist', error: e.message });
    }
});
app.get('/api/stores/:id/pdvs', async (req, res) => {
    try {
        const db = await dbPromise;
        const pdvs = await db.all('SELECT * FROM pdvs WHERE storeId = ?', [req.params.id]);
        res.json(pdvs);
    } catch (err) {
        res.status(500).json({ message: 'Erro ao buscar PDVs da loja', error: err.message });
    }
});
app.get('/api/checklists/history', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Erro ao buscar histórico de checklists.', error: e.message });
    }
});
...
app.get('/api/checklists/:id', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Erro ao buscar detalhes do checklist.', error: e.message });
    }
});
...
app.post('/api/logs/admin', async (req, res) => {
    ...
    try {
        const db = await dbPromise;
        await db.run('INSERT INTO actionLogs (description, metadata, timestamp, userId, userName) VALUES (?, ?, ?, ?, ?)', [description, JSON.stringify(metadata), new Date().toISOString(), userId, userName]);
        res.status(201).json({ success: true });
    } catch (e) { res.status(500).json({ message: 'Erro ao gravar log', error: e.message }); }
});
app.get('/api/logs/admin', async (req, res) => {
    try {
        const db = await dbPromise;
        const logs = await db.all('SELECT * FROM actionLogs ORDER BY timestamp DESC');
        res.json(logs);
    } catch (e) { res.status(500).json({ message: 'Erro ao buscar logs', error: e.message }); }
});
app.get('/api/logs/pdv', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Erro ao buscar logs de PDV.', error: e.message });
    }
});
