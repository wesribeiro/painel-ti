...
async function initializeDatabase() {
    ...
    if (usersCount.count === 0) {
        ...
        try {
            ...
            const roleStmt = await db.prepare('INSERT INTO roles (id, name, permissions) VALUES (?, ?, ?)');
            for (const role of roles) {
                await roleStmt.run(role.id, role.name, JSON.stringify(role.permissions));
            }
            ...
            const storeStmt = await db.prepare('INSERT INTO stores (id, name, pdvNamingStart, checklistConfig) VALUES (?, ?, ?, ?)');
            for (const store of stores) {
                await storeStmt.run(store.id, store.name, store.pdvNamingStart, JSON.stringify(store.checklistConfig));
            }
            await storeStmt.finalize();
            
            // Users
             const users = [
                { id: 1, name: 'Administrador', username: 'admin', password: 'Nilo@@1254', roleId: 1, storeId: null },
                { id: 2, name: 'Fulano de Tal', username: 'fulano', password: '123', roleId: 2, storeId: 1 },
                { id: 3, name: 'Ciclano Supervisor', username: 'supervisor', password: null, roleId: 3, storeId: null },
            ];
            const userStmt = await db.prepare('INSERT INTO users (id, name, username, password, roleId, storeId) VALUES (?, ?, ?, ?, ?, ?)');
            for (const user of users) {
                const passwordHash = user.password ? await bcrypt.hash(user.password, SALT_ROUNDS) : null;
                await userStmt.run(user.id, user.name, user.username, passwordHash, user.roleId, user.storeId);
            }
            ...
            const pdvStmt = await db.prepare('INSERT INTO pdvs (id, storeId, number) VALUES (?, ?, ?)');
            ...
            // PDV Items
            const pdvItems = [ { name: 'Monitor'}, { name: 'Teclado'}, { name: 'Mouse'}, { name: 'Scanner de Mão'}, { name: 'Impressora Fiscal'} ];
            const pdvItemStmt = await db.prepare('INSERT INTO pdvItems (name) VALUES (?)');
            ...
            // Status Types
            const statusTypes = [
                { name: 'Ok', color: 'green' }, { name: 'Atenção', color: 'orange' },
                { name: 'Manutenção', color: 'red' }, { name: 'Reserva', color: 'gray' },
                { name: 'Sem status', color: 'gray' },
            ];
            const statusTypeStmt = await db.prepare('INSERT INTO statusTypes (name, color) VALUES (?, ?)');
            ...
            // Status History
            const statusHistory = [
                { pdvId: 101, statusId: 3, description: 'PDV não liga, fonte.', techId: 2, timestamp: '2025-09-25T10:00:00.000Z', itemId: null },
                { pdvId: 101, statusId: 1, description: 'Fonte trocada', techId: 2, timestamp: '2025-09-26T11:30:00.000Z', itemId: null },
                { pdvId: 102, statusId: 2, description: 'Tecla "5" não funciona', techId: 2, timestamp: '2025-09-27T09:00:00.000Z', itemId: 2 },
            ];
            const historyStmt = await db.prepare('INSERT INTO statusHistory (pdvId, statusId, description, techId, timestamp, itemId) VALUES (?, ?, ?, ?, ?, ?)');
            for(const entry of statusHistory) {
                await historyStmt.run(entry.pdvId, entry.statusId, entry.description, entry.techId, entry.timestamp, entry.itemId);
            }
